...
module.exports = function printFailures(failures, options) {
  var byContext = {};
  ...
  Object.keys(byContext).forEach(function(context) {
    ...
    byContext[context].forEach(printFailure.bind(null, options));
  });
};

function printFailure(options, failure) {
  ...
  function printFrame(frame) {
    ...
    var fmt = formatLocation(location, options);
    if (fmt) {
      console.error(fmt);
    }
  }
